...
class Requestpembelian extends Model
{
    ...
    {
        return $this->belongsTo(Memo::class,'no_request');
    }
    ...
    public static function boot()
    {
        ...
        static::creating(function ($query){
            ...
            $query->no_request = static::generateKode(request()->tanggal_pembelian, request()->jenis_po);
    ...
    {
        $konek = static::konek();
        $kode = TransaksiSetup::where('kode_setup','034')->first();
        ...
        $lastRecort = self::on($konek)->where($primary_key,'like',$prefix_result.'%')->orderBy('no_request', 'desc')->first();
