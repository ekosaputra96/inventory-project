...
class DisassemblingController extends Controller
{
    public function index()
    {
        ...
        return view('admin.disassembling.index',compact('period', 'nama_lokasi','nama_company','Produk'));
    }
    ...
    public function anyData()
    {
        ...
        if($lokasi == 'HO'){
            return Datatables::of(Disassembling::on($konek)->with('produk')->orderBy('created_at','desc'))->make(true);
        }else{
            return Datatables::of(Disassembling::on($konek)->with('produk')->orderBy('created_at','desc')->where('kode_lokasi', auth()->user()->kode_lokasi))->make(true);
        }
    }

    public function exportPDF(Disassembling $disassembling){
        $request = $_GET['id'];
        $konek = self::konek();
        $disassembling = Disassembling::on($konek)->where('id',$request)->first();
        $user = $disassembling->created_by;
        $id = $disassembling->id;

        $kode_company = $disassembling->kode_company;
        $disassemblingdetail = DisassemblingDetail::on($konek)->where('id',$request)->get();
        ...
        $tgl = $disassembling->tanggal;
        $date=date_create($tgl);

        $pdf = PDF::loadView('/admin/disassembling/pdf', compact('disassemblingdetail','request','tgl', 'id','nama_company','date_now','disassembling','user'));
        $pdf->setPaper([0, 0, 684, 792], 'potrait');

        return $pdf->stream('Laporan Disassembling '.$id.'.pdf');        
    }
    ...
    public function exportPDF3(){
        ...
        $disassembling = Disassembling::on($konek)->find($request);
        $grand_total1 = $disassembling->qty_disassembling * $disassembling->hpp;
        $jur = $disassembling->kode_jurnal;
        $jurnal = Jurnal::find($jur);

        $total_qty = 0;
        $total_harga = 0;
        $grand_total = 0;
        $detail = DisassemblingDetail::on($konek)->where('id',$request)->get();
        ...
        $pdf = PDF::loadView('/admin/disassembling/pdf2', compact('disassembling','request', 'jurnal','tgl','date', 'ttd','nama_company','date_now','ledger','ledger2','dt','user','nama','nama2','journal_date','grand_total','grand_total1'));
        $pdf->setPaper([0, 0, 684, 792], 'potrait');
    ...
    public function Showdetail()
    {
        ...
        $total_harga = 0;
        $grand_total = 0;
        $disassemblingdetail= DisassemblingDetail::on($konek)->with('produk')->where('id',request()->id)
        ->orderBy('created_at', 'desc')->get();

        $output = array();

        foreach ($disassemblingdetail as $row){
            $total_qty += $row->qty;
            $subtotal = $row->hpp * $row->qty;
            $total_harga += $subtotal;
    ...
    public function detail($disassembling)
    {   
        $konek = static::konek();
        $disassembling = Disassembling::on($konek)->with('produk')->find($disassembling);
        $tanggal = $disassembling->tanggal;
        $id = $disassembling->id;
        ...
        if($validate == true){
            $data = Disassembling::on($konek)->find($id);
            $status = $disassembling->status;

            $total_qty = 0;
            $total_harga = 0;
            $grand_total = 0;

            $disassemblingdetail = DisassemblingDetail::on($konek)->with('produk')->where('id', $disassembling->id)
            ->orderBy('created_at','desc')->get();
            ...
            $list_url= route('disassembling.index');
            ...
            return view('admin.disassemblingdetail.index', compact('disassembling','disassemblingdetail','list_url','Produk','Satuan','total_qty','grand_total','Company','period', 'nama_lokasi','nama_company'));
        }
        else{
    ...
    function produkChecker($id, $tahun, $bulan, $tanggal_baru, $tgl, $disassembling, $koneksi)
    {
        $konek = static::konek();
        $disassemblingdetail = DisassemblingDetail::on($konek)->with('produk')->where('id', request()->id)->get();
        ...
        if(!empty($disassemblingdetail)){
            foreach ($disassemblingdetail as $rowdata){
                $data[] = array(
                    'id'=>$id,
        ...
        if(!empty($disassemblingdetail)){
            ...
            for($i = 0; $i < $leng; $i++){
                ...
                if($tb_item_bulanan != null){
                    ...
                    $disassemblingdetail2 = DisassemblingDetail::on($konek)->where('id', $id)->where('kode_produk',$data[$i]['kode_produk'])->where('partnumber',$data[$i]['partnumber'])->first();

                    $harga = $data[$i]['harga'];
                    $qty_baru = $data[$i]['qty'];

                    $ass_stok_new = $assembling_stock + $qty_baru;
                    $ass_amount_new = $assembling_amount + ($harga * $qty_baru);
                    $end_stok_new = $stock_begin + $stok_in - $stock_out - $stock_sale + $stock_trfin - $stock_trfout + $stock_op + $stock_adjustment - $retur_beli_stock + $retur_jual_stock - $disassembling_stock + $ass_stok_new;
                    $end_amount_new = $amount_begin + $amount_masuk - $amount_keluar - $amount_sale + $amount_trfin - $amount_trfout + $amount_op + $amount_adjustment - $retur_beli_amount + $retur_jual_amount - $disassembling_amount + $ass_amount_new;
                    ...
                    $tgl_diss1 = $disassembling->tanggal;
                    $tahun_transaksi1 = Carbon\Carbon::createFromFormat('Y-m-d',$tgl_diss1)->year;
                    $bulan_transaksi1 = Carbon\Carbon::createFromFormat('Y-m-d',$tgl_diss1)->month;
                    ...
                    if($status_reopen == 'true'){
                        $tgl_diss = $disassembling->tanggal;
                        $tahun_transaksi = Carbon\Carbon::createFromFormat('Y-m-d',$tgl_diss)->year;
                        $bulan_transaksi = Carbon\Carbon::createFromFormat('Y-m-d',$tgl_diss)->month;
                        ...
                        $j = 1;
                        while($j <= $final_month){
                            $disassemblingdetail = DisassemblingDetail::on($konek)->where('id', $id)->where('kode_produk',$data[$i]['kode_produk'])->where('partnumber',$data[$i]['partnumber'])->first();

                            $hpp = $disassemblingdetail->hpp;

                            $stock_ass = $data[$i]['qty'];
                            $amount_ass = $hpp*$stock_ass;
    ...
    function produkChecker2($id, $tahun, $bulan, $tanggal_baru, $tgl, $disassembling, $koneksi)
    {
        $konek = self::konek();
        $disassemblingdetail = DisassemblingDetail::on($konek)->with('produk')->where('id', request()->id)->get();
        $id = request()->id;
    ...
    public function cekjurnal2()
    {
        ...
        $cek_ar = Disassembling::on($konek)->where('no_journal', request()->no_journal)->first();
    ...
    public function posting()
    {
        ...
        if($cek_bulan == null || $level == 'superadministrator' || $level == 'user_rince' || $level == 'user_thomas'){
            if($cek_company == '04' || $cek_company == '0401' || $cek_company == '03' || $cek_company == '05' || $cek_company == '02' || $cek_company == '06'){
                $disassemblingdetail = DisassemblingDetail::on($konek)->where('id', request()->id)->get();
                $leng = count($disassemblingdetail);
                $data = array();
            ...
            $disassembling = Disassembling::on($konek)->find(request()->id);
            $cek_status = $disassembling->status;
            if($cek_status != 'OPEN'){  
                $message = [
                    'success' => false,
                    'title' => 'Simpan',
                    'message' => 'POST Disassembling, id : '.$disassembling->id.' sudah dilakukan! Pastikan Anda tidak membuka menu DISASSEMBLING lebih dari 1',
                ];
                return response()->json($message);
            }

            $id = $disassembling->id;
            $crate_disassembling = $disassembling->created_at;
            $koneksi = $disassembling->kode_lokasi;
            ...
            $validate_produk = $this->produkChecker($id, $tahun, $bulan, $tanggal_baru, $tgl, $disassembling, $koneksi);

            if($validate_produk == true){
                $tb_item_bulanan = tb_item_bulanan::on($konek)->where('kode_produk',$disassembling->kode_produk)->where('kode_lokasi',$koneksi)->where('partnumber',$disassembling->partnumber)->whereMonth('periode', $bulan)->whereYear('periode', $tahun)->first();
                ...
                if($tb_item_bulanan != null){
                    ...
                    $produk = Produk::on($konek)->find($disassembling->kode_produk);

                    $disassemblingdetail2 = DisassemblingDetail::on($konek)->where('id', $id)->where('kode_produk',$disassembling->kode_produk)->where('partnumber',$disassembling->partnumber)->first();

                    $harga = $disassembling->hpp;
                    $qty_baru = $disassembling->qty_disassembling;
                    ...
                    $update_item_bulanan = tb_item_bulanan::on($konek)->where('kode_produk',$produk_awal)->where('kode_lokasi',$koneksi)->where('partnumber',$disassembling->partnumber)->whereMonth('periode', $bulan)->whereYear('periode', $tahun)->update($tabel_baru);
                }

                $tabel_history = [
                    'kode_produk'=>$produk_awal,
                    'no_transaksi'=>'Disassembling, id : '.$id,
                    'tanggal_transaksi'=>$tgl,
                    'jam_transaksi'=>$crate_disassembling,
                    'qty_transaksi'=>$disassembling->qty_disassembling,
                    'harga_transaksi'=>$disassembling->hpp,
                    'total_transaksi'=>$disassembling->qty_disassembling*$disassembling->hpp,
                    'kode_lokasi'=>$koneksi,
                ];
                ...
                if($status_reopen == 'true'){
                    ...
                    $j = 1;
                    while($j <= $final_month){
                        $disassemblingdetail = DisassemblingDetail::on($konek)->where('id', $id)->where('kode_produk',$disassembling->kode_produk)->where('partnumber',$disassembling->partnumber)->first();

                        $hpp = $disassembling->hpp;

                        $stock_dis = $disassembling->qty_disassembling;
                        ...
                        $tb_item_bulanan2 = tb_item_bulanan::on($konek)->where('kode_produk',$disassembling->kode_produk)->where('partnumber',$disassembling->partnumber)->where('kode_lokasi',$koneksi)->whereMonth('periode',$bulan2)->whereYear('periode', $tahun2)->first();
                        ...
                        if($tb_item_bulanan2 != null){
                            ...
                            $update_item_bulanan2 = tb_item_bulanan::on($konek)->where('kode_produk',$disassembling->kode_produk)->where('partnumber',$disassembling->partnumber)->where('kode_lokasi',$koneksi)->whereMonth('periode',$bulan2)->whereYear('periode', $tahun2)->update($tabel_baru2);
                        }
                        $j++;
                    }
                }
                    ...
                    for($i = 0; $i < $leng; $i++){
                        ...
                        if($tb_item_bulanan != null){
                            ...
                            $tabel_baru = [
                                ...
                                'assembling_stock'=>$data[$i]['qty'],
                                'assembling_amount'=>$diss_amt,
                ...
                $nama = auth()->user()->name;
                $tmp = ['nama' => $nama,'aksi' => 'Post Disassembling, id : '.$id.'.','created_by'=>$nama,'updated_by'=>$nama];
                ...
                //     $total_harga = 0;
                //     $grand_total = 0;
                //     $detail = DisassemblingDetail::on($konek)->where('id',$disassembling->id)->get();
                //     foreach ($detail as $row){
                //         $total_qty += $row->qty;
                //         $subtotal = $row->hpp * $row->qty;
                //         $total_harga += $subtotal;
                ...
                //         $bulan = Carbon\Carbon::parse($disassembling->tanggal)->format('m');
                //         $tahun = Carbon\Carbon::parse($disassembling->tanggal)->format('Y');
                ...
                //     $cek_produk2 = Produk::on($konek)->where('id', $disassembling->kode_produk)->first();
                //     if($cek_produk2->kode_kategori == 'APD'){
                //         $gt_apd2 += $disassembling->qty_disassembling * $disassembling->hpp;
                //     }

                //     if($cek_produk2->kode_kategori == 'BAN'){
                //         $gt_ban2 += $disassembling->qty_disassembling * $disassembling->hpp;
                //     }

                //     if($cek_produk2->kode_kategori == 'BBM'){
                //         $gt_bbm2 += $disassembling->qty_disassembling * $disassembling->hpp;
                //     }

                //     if($cek_produk2->kode_kategori == 'OLI'){
                //         $gt_oli2 += $disassembling->qty_disassembling * $disassembling->hpp;
                //     }

                //     if($cek_produk2->kode_kategori == 'SPRT'){
                //         $gt_sprt2 += $disassembling->qty_disassembling * $disassembling->hpp;
                //     }

                //     if($cek_produk2->kode_kategori == 'UNIT'){
                //         $gt_unit2 += $disassembling->qty_disassembling * $disassembling->hpp;
                //     }
                ...
                //                 'periode'=>$bulan,
                //                 'account'=>$coa_inventory->account,
                //                 'no_journal'=>$disassembling->no_journal,
                //                 'journal_date'=>$disassembling->tanggal,
                //                 'db_cr'=>'D',
                //                 'reference'=>$disassembling->id,
                //                 'debit'=>$gt_apd,
                //                 'kode_lokasi'=>$lokasi,
                //             ];
                ...
                //             $type = 'Inventory';
                //             $transaksi = $disassembling;
                //             $tgl_trans = $disassembling->tanggal;
                //             $harga_acc = $gt_apd;
                //             $dbkr = 'D';
        ...
        if($cek_bulan == null || $level == 'superadministrator' || $level == 'user_rince' || $level == 'user_thomas'){
            ...
            if($cek_status != 'POSTED'){  
                $message = [
                    'success' => false,
                    'title' => 'Simpan',
                    'message' => 'UNPOST Disassembling, id : '.$disassembling->id.' sudah dilakukan! Pastikan Anda tidak membuka menu DISASSEMBLING lebih dari 1',
                ];
                return response()->json($message);
            }
            ...
            $validate_produk = $this->produkChecker2($id, $tahun, $bulan, $tanggal_baru, $tgl, $disassembling, $koneksi);
            ...
            if($validate_produk == true){
                ...
                if(!empty($disassemblingdetail)){
                    ...
                    for($i = 0; $i < $leng; $i++){
                        ...
                        if($tb_item_bulanan != null){
                            ...
                            $update_produk_history = tb_produk_history::on($konek)->where('no_transaksi','Disassembling, id : '.$id)->delete();
                ...
                $disassembling = Disassembling::on($konek)->find(request()->id);
                $disassembling->status = "OPEN";
                $disassembling->save(); 

                $nama = auth()->user()->name;
                $tmp = ['nama' => $nama,'aksi' => 'Unpost Disassembling, id : '.$id.'.','created_by'=>$nama,'updated_by'=>$nama];
                             //dd($tmp);
                user_history::on($konek)->create($tmp);
                ...
                if($cek_company == '04' || $cek_company == '0401' || $cek_company == '03' || $cek_company == '0501'){
                    ...
                    $get_ledger = Ledger::on($konek2)->where('no_journal',$disassembling->no_journal)->get();
                    ...
                    $update_ledger = Ledger::on($konek2)->where('no_journal',$disassembling->no_journal)->delete();
                }
    ...
    public function store(Request $request)
    {
        ...
        if($validate == true){
            ...
            $disassembling = Disassembling::on($konek)->create($request->all());

            $no = Disassembling::on($konek)->orderBy('created_at','desc')->first();
            $nama = auth()->user()->name;
            $tmp = ['nama' => $nama,'aksi' => 'Simpan No. Disassembling: '.$no->id.'.','created_by'=>$nama,'updated_by'=>$nama];
            //dd($tmp);
            user_history::on($konek)->create($tmp);
    ...
    public function edit_disassembling()
    {
        $konek = self::konek();
        $id = request()->id;
        $data = Disassembling::on($konek)->find($id);
        $output = array(
            ...
            'hpp'=> $data->hpp,
            'qty_disassembling'=> $data->qty_disassembling,
    ...
    public function updateAjax(Request $request)
    {
        ...
        if($validate == true){
            $disassembling = Disassembling::on($konek)->find($request->id)->update($request->all());

            $nama = auth()->user()->name;
            $tmp = ['nama' => $nama,'aksi' => 'Edit No. Disassembling: '.$request->id.'.','created_by'=>$nama,'updated_by'=>$nama];
            //dd($tmp);
            user_history::on($konek)->create($tmp);
    ...
    public function hapus_disassembling()
    {   
        $konek = self::konek();
        $level = auth()->user()->level;
        ...
        if($validate == true){
            ...
            $nama = auth()->user()->name;
            $tmp = ['nama' => $nama,'aksi' => 'Hapus No. Disassembling: '.$id.'.','created_by'=>$nama,'updated_by'=>$nama];
            ...
            $message = [
                'success' => true,
                'title' => 'Update',
                'message' => 'Data ['.$data->id.'] telah dihapus.'
            ];
            return response()->json($message);
