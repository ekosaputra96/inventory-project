...
class RequestpembelianController extends Controller
{
    ...
    public function exportPDF(){
        $konek = self::konek();
        $request = $_GET['no_request'];

        $reqpembelian = Requestpembelian::on($konek)->find($request)->first();
        $user = $reqpembelian->created_by;
        
        $dt = Carbon\Carbon::now(); 
        $date_now = Carbon\Carbon::parse($dt)->format('d/m/Y H:i:s');

        $reqdetail = RequestProduk::on($konek)->where('no_request',$request)->get();
        $leng = count($reqdetail);
        ...
        $pdf = PDF::loadView('/admin/requestpembelian/pdf', compact('reqdetail','request', 'nama', 'reqpembelian', 'user', 'ttd','total_qty','date_now','konek'));
        $pdf->setPaper([0, 0, 684, 792], 'potrait');
            
        $tmp = ['nama' => $nama,'aksi' => 'Print No. Request: '.$request.'.','created_by'=>$nama,'updated_by'=>$nama];
    ...
    public function Post()
    {
        ...
        $nama = auth()->user()->name;
        $tmp = ['nama' => $nama,'aksi' => 'Post No. Request: '.$req->no_request.'.','created_by'=>$nama,'updated_by'=>$nama];
        user_history::on($konek)->create($tmp);
    ...
    public function Unpost()
    {
        ...
        $nama = auth()->user()->name;
        $tmp = ['nama' => $nama,'aksi' => 'Unpost No. Request: '.$req->no_request.'.','created_by'=>$nama,'updated_by'=>$nama];
        user_history::on($konek)->create($tmp);
