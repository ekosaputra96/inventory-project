...
class CheckmonthlypartController extends Controller
{
    ...
    public function index()
    {
        ...
        if($level == 'superadministrator' || $level == 'user_rince' || $level == 'user_herry'){
            return view('admin.checkmonthlypart.index',compact('create_url','tanggal','period', 'nama_lokasi'));
        }
        else{
            return view('admin.checkmonthlypart.blank',compact('create_url','tanggal','period', 'nama_lokasi'));
        }
    ...
    {
        $konek = self::konek();
        $item = tb_item_bulanan::on($konek)->whereMonth('periode', $request->bulan_akhir)->whereYear('periode', $request->tahun_awal)->get();
        $total = count($item);
        $part = array();
        $a = 0;
    ...
    {   
        ...
        $get_bulanan = tb_item_bulanan::on($konek)->where('periode', $tanggal_baru1)->orderBy('id','asc')->get();
        ...
        $tb_item_bulanan = tb_item_bulanan::on($konek)->where('periode', $tanggal_baru1)->orderBy('id','asc')->skip($lewat)->take($sisa)->get();
