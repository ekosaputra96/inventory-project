...
class PemakaianController_lama extends Controller
{
    public function index()
    {
        ...
        if (auth()->user()->kode_company == '02'){
            $Workorder = Workorder::on($konek)->where('status','POSTED')->pluck('no_wo','no_wo');
        }else {
            $Workorder = '';
        }
    ...
    public function anyData()
    {
        ...
        if($lokasi == 'HO'){
            ...
            return Datatables::of(Pemakaian::on($konek)->with('mobil','jenismobil','alat','kapal','company','Lokasi')->orderBy('created_at','desc')->withCount('pemakaiandetail')->where('kode_lokasi', auth()->user()->kode_lokasi))->make(true);
        }
    }
    ...
    {   
        ...
        if($validate == true){
            ...
            //     $Produk = Produk::on($konek)->Join('workorder_detail', 'produk.id', '=', 'workorder_detail.kode_produk')->where('workorder_detail.type','Stock')->pluck('produk.nama_produk','produk.id');
            // }else {
    ...
    public function export2(){
        ...
        return $pdf->stream('Laporan Pemakaian '.$no_pemakaian.'.pdf');
    }
